...
let test20 = {
  componentDidMount: `useEffect(() => {
    let myName = axios.get(/api/name/1)
    this.setState({
      name: myName
    })
    let myFriend = this.state.friend
    axios.get(/api/myFriend)
    let myNum = 1
    let anotherNum = myNum++
    let string = anotherNum.toString()
    'random line'
    'another random line'
  }, [])`,
}
const iterator = (obj) => {
  let effectsArr = []
  for (let key in obj) {
    let inside = getInsideOfFunc(obj[key], 'useEffect')
...
function groupStatements(variablesObj, statementsArr) {
  ...
  for (let key in variablesObj) {
    ...
    for (let variable of variablesObj[key].variables) {
      ...
      // for (let statement of statementsArr) {
  ...
  // console.log(groupedStatementsArr)
