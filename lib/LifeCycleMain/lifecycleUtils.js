...
function createUseEffects(objectOfLifeCycle) {
  ...
  if (objectOfLifeCycleKeys.length === 1) {
    ...
    if (objectOfLifeCycleKeys.includes('componentWillUnmount()')) {
      ...
      // if (
      //   getInsideOfFunc(
      //     objectOfLifeCycle['componentWillUnmount()'],
      //     'componentWillUnmount()'
      //   ) === ' '
      // ) {
      //   delete objectOfLifeCycle['componentWillUnmount()'];
      // }
      if (checkUnmount !== -1) {
...
function multiuseEffectCreate(object, cleanup) {
  ...
  if (cleanup) {
    ...
    let UnmountArrayCleanup = lifecycleObject['componentWillUnmount()'].split(
      ' '
    );

    UnmountArrayCleanup = UnmountArrayCleanup.filter((line) => {
      return line.indexOf(UnMountParentheses) !== -1;
    });

    const cleanUpString = createCompWillUnmountCleanUp(
      UnmountArrayCleanup.join('')
    );
    ...
    if (lifecycleObject['componentWillUnmount()'].split(' ').length < 1) {
