...
function translateToFunctionComp(fileInString) {
  ...
  // console.log(funcs)
  const lifeCyclesObj = findComponents(lifeCyclesStr)
  const actualObj = createUseEffects(lifeCyclesObj)
  const numberOfKeys = Object.keys(lifeCyclesObj)
  // const arrOfUseEffects = createMultipleUseEffects(lifeCyclesObj)
  console.log('baileys', actualObj)
  const arrOfUseEffects = iterator(actualObj)
  console.log('cams', arrOfUseEffects)
